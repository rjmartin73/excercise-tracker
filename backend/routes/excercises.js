...
router.route('/').get((req, res) => {
	Excercise.find().then((excercises) => res.json(excercises)).catch((err) => res.status(400).json(`Error: ${err}`));
});
...
router.route('/add').post((req, res) => {
	...
	newExcercise.save().then(() => res.json('Excercise added!')).catch((err) => res.status(400).json(`Error: ${err}`));
});
